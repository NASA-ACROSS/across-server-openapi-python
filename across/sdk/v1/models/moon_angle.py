...
MOONANGLE_ANY_OF_SCHEMAS = ["List[float]", "float"]

class MoonAngle(BaseModel):
    """
    Angular distance between the Moon and the coordinate
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = MoonAngle.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in MoonAngle with anyOf schemas: List[float], float. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MoonAngle with anyOf schemas: List[float], float. Details: " + ", ".join(error_messages))
        else:
            return instance
